    ...
    public class PhaseShifterStopwatch : PlayerItem
    {
        ...
        protected override void DoEffect(PlayerController user)
        {
            if (this.hasTriggeredVFX != true)
            {
                ...
                this.hadCrest = user.HasPickupID(305);
        ...
        protected override void DoActiveEffect(PlayerController user)
        {
            ...
            if (teleportObject != null)
            {
                ...
                if (user.healthHaver.GetCurrentHealth() < this.health) user.healthHaver.ForceSetCurrentHealth(this.health);
                if (user.healthHaver.Armor < this.armor) user.healthHaver.Armor = this.armor;
                if (this.hadCrest) user.GiveItem("old_crest");
                EndEffect(user);
